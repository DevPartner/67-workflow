...
def subprocess_script_runner(
    ...
    logger.info("[skill:%s] running script: %s  args: %s", skill.name, script.name, args)
    ...
    try:
        ...
        stripped = output.strip() or "(no output)"
        logger.info("[skill:%s] script output added to context:\n%s", skill.name, stripped)
        return stripped
